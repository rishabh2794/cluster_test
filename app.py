...
ward_file = st.file_uploader(
    "Upload WARD boundary file (GeoJSON/JSON/KML, optional)",
...
subcategory_option = st.selectbox(
    "Choose issue subcategory to analyze:", subcategory_options
)
...
if csv_file:
    try:
        ...
        if not required_cols.issubset(df.columns):
            ...
            eps_rad = radius_m / 6371000  # Earth radius in meters
            ...
            if ward_file:
                with tempfile.NamedTemporaryFile(delete=False,
                                                 suffix="." + ward_file.name.split(".")[-1]) as tmp:
                ...
                gdf_points = gpd.GeoDataFrame(
                    clustered,
                    geometry=gpd.points_from_xy(clustered['LONGITUDE'], clustered['LATITUDE']),
                    crs="EPSG:4326"
                )
            ...
            # Hyperlink before/after photos if URLs
            wb = load_workbook(excel_filename)
            ws = wb['Clustering Application Summary']
            for row in range(2, ws.max_row + 1):
                for col_idx in (11, 12):  # BEFORE PHOTO, AFTER PHOTO
            ...
            m = folium.Map(
                location=[summary_sheet['LATITUDE'].mean(), summary_sheet['LONGITUDE'].mean()],
                zoom_start=13
            )
            if ward_file:
